...
const preparationContainer = document.getElementById("element-prep")
 ...
 let count = 0;
function addPreparationStep(preparation) {
    preparationContainer.innerHTML = "";
    const preparationArray = JSON.parse(preparation);

    preparationArray.forEach((item, index) => {
        ...
        inputPreparation.innerHTML = `
            ...
            <div class="img-preparation">
                ...
                ${item.image ? `<img class="img-prepa" src="${item.image}" alt="Foto del plato preparación" />`: ''}
                </div>
                <button class="buton-img-file" type="button">
                <img class="img-prepa" src="/src/images/camara.png" alt="Foto del plato preparación" />  
                </button>
                </div>
            </div>
        `;
